jQuery( document ).ready( function( $ ) {
	...
	$( '.ci-cf-wrap .nozama-lite-alpha-color-picker' ).alphaColorPicker();
	...
	$( 'body' ).on( 'click', '.ci-media-button', function( e ) {
		...
		var ciMediaUpload = wp.media( {
			...
			title: bMulti == true ? nozama_lite_plugin_PostMeta.tSelectFiles : nozama_lite_plugin_PostMeta.tSelectFile,
			button: {
				text: bMulti == true ? nozama_lite_plugin_PostMeta.tUseTheseFiles : nozama_lite_plugin_PostMeta.tUseThisFile
			},
			multiple: bMulti
		} ).on( 'select', function(){
				...
				*/
				...
				if( bMulti == false ) {
					...
					if( target_preview.length > 0 ) {
						...
						var html = '<img src="' + attachment.sizes.thumbnail.url + '" /><a href="#" class="close media-modal-icon" title="' + nozama_lite_plugin_PostMeta.tRemoveImage + '"></a>';
	...
	$( 'body' ).on( 'click', '.ci-upload-to-gallery', function( e ) {
		...
		var ciMediaGallery = wp.media( {
			frame: 'select',
			title: nozama_lite_plugin_PostMeta.tSelectFiles,
			button: {
				text: nozama_lite_plugin_PostMeta.tUseTheseFiles
			},
			multiple: true
			...
			nozama_lite_featgal_AJAXUpdate( target_parent );
	...
	function nozama_lite_featgal_UpdateIDs( preview_element ) {
		var ids = [];
	...
	function nozama_lite_featgal_AJAXUpdate( gallery_container ) {
		...
		$.ajax( {
			type: 'post',
			url: nozama_lite_plugin_PostMeta.ajaxurl,
			data: {
				action: 'nozama_lite_plugin_featgal_AJAXPreview',
				ids: target_ids.val()
			},
			dataType: 'text',
			beforeSend: function() {
				target_preview.empty().html( '<p>' + nozama_lite_plugin_PostMeta.tLoading + '</p>' );
			},
			success: function( response ) {
				if ( response == 'FAIL' ) {
					target_preview.empty().html( '<p>' + nozama_lite_plugin_PostMeta.tPreviewUnavailable + '</p>' );
				} else {
					...
					$.each( response, function( key, value ) {
						$( '<div class="thumb"><img src="' + value.url + '" data-img-id="' + value.id + '"><a href="#" class="close media-modal-icon" title="' + nozama_lite_plugin_PostMeta.tRemoveFromGallery + '"></a></div>' ).appendTo( target_preview );
					} );
				}
			}//success
		});//ajax
	...
	$( 'body' ).on( 'click', '.ci-media-manager-gallery .thumb a.close', function( event ) {
		...
		nozama_lite_featgal_UpdateIDs( container );
	} );
	...
	if ( preview_areas.length > 0 ) {
		preview_areas.sortable( {
			update: function( event, ui ) {
				nozama_lite_featgal_UpdateIDs( $( this ) );
			}
		} );
	}
